...
export default class extends Controller {
  ...
  release() {
    ...
    for (const toast of this.toastElements) {
      if (toast.dataset.state === "open") this.start(toast)
    }
  }

  get toastElements() {
    return [ ...this.listTarget.querySelectorAll("[data-slot=toast]") ]
  }
  ...
  place() {
    ...
    front.forEach((toast, index) => {
      ...
      toast.style.transform = `translateY(${this.lift * offset}px) scale(${this.expanded || !behind ? 1 : 1 - index * 0.05})`
      toast.style.height = this.expanded || !behind ? "" : `${frontHeight}px`
      toast.style.opacity = behind && index > 2 && !this.expanded ? "0" : "1"
      toast.style[this.lift > 0 ? "top" : "bottom"] = "0px"
    ...
    const total = this.expanded ? stacked + GAP * (front.length - 1) : frontHeight + GAP * Math.min(front.length - 1, 2)
    this.listTarget.style.height = `${total}px`
  }
