...
// The chart is drawn by the server — every slice is already in the document,
// with its label and the value it should show. This is the half a static SVG
// cannot do: follow the pointer with the tooltip.
//
// Nothing here computes anything about the data. It reads what the slice
// carries and moves a panel, which is why a chart of a hundred slices costs the
// same as a chart of three.
export default class extends Controller {
  static targets = [ "slice", "tooltip", "label", "name", "value", "indicator" ]
  ...
  fill(slice) {
    const { label, display, key } = slice.dataset

    if (this.hasLabelTarget) this.labelTarget.textContent = label
    if (this.hasNameTarget) this.nameTarget.textContent = label
    if (this.hasValueTarget) this.valueTarget.textContent = display
    // The swatch takes the slice's own fill rather than a colour repeated in
