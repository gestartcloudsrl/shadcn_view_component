...
The time in upstream's Date-and-Time example is an `<input type="time">` and not
a component here, but one thing about it is worth knowing and was measured
rather than assumed:
...
segments still accept any digits, and the complaint arrives afterwards. Where
only a quarter-hour may be chosen at all, the control is a `<select>`.
