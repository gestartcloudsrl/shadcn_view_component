...
RSpec.describe "rendered output snapshots" do
  # The calendar renders today: a `data-today` cell, a `Today, …` label and the
  # date the controller counts presets from. Without a fixed clock these
  # fixtures go red at midnight and are regenerated by whoever runs the suite
  # next, which is a golden file that means nothing. Frozen rather than stubbed,
  # per the house rules, and to a date the fixtures were taken on.
  ...
  def self.normalize(html)
    # Every generated id in this gem is `shadcn-<part>-<eight hex>`, so the
    # rule is the convention rather than a list that grows a name each time a
    # component with an id is added — which it did three times in three
    # branches.
    html.gsub(/\b(shadcn-[a-z-]+-)[0-9a-f]{8}\b/, '\1x')
